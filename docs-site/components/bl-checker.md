...
BlChecker组件，用于blchecker场景。

## 📦 引入

```typescript
import { BlChecker } from '@/uni_modules/belay-unix'
```
...
const basicCode = `<template>
  <view>
    <bl-checker></bl-checker>
  </view>
<\/template>`

const disabledCode = `<template>
  <view>
    <bl-checker disabled>禁用状态</bl-checker>
  </view>
<\/template>`

const eventCode = `<template>
  <view>
    <bl-checker @click="handleClick">点击我</bl-checker>
  </view>
<\/template>
...
`uni_modules/belay-unix/components/bl-checker/README.md`
